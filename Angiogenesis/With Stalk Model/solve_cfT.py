    ...
    for y in range(0,set['Ny']+1,2):
        for x in range(0,set['Nx']+1,2):
            if y == set['Ny']:
                if not x == 0:
                    if not x == set['Nx']:
                        cijx_p, cijx_n, cijy_p, cijy_n = max_min_c(set,sol,x,y)
                        bijx_p, bijx_n, bijy_p, bijy_n = max_min_b(set,sol,x,y)
                        G_plus = chemo_coef*cijx_p-coef['Si']*bijx_p
                                    
                        cijx_p, cijx_n, cijy_p, cijy_n = max_min_c(set,sol,x+2,y)
                        bijx_p, bijx_n, bijy_p, bijy_n = max_min_b(set,sol,x+2,y)
                        G_neg = chemo_coef*cijx_p-coef['Si']*bijx_p
                        
                        chemo_coef = coef['Ki_n']/(1+coef['Al_n']*1/4*(sol['c'][xb-2,yb-2]+sol['c'][xb-2,yb]+sol['c'][xb,yb-2]+sol['c'][xb,yb]))
                        
                        F_sol_1[x,y] = -coef['D_n']/(set['h'])*(sol['n'][x+1,y-1]-sol['n'][x-1,y-1])+sol['n'][x-1,y-1]*G_plus-sol['n'][x+1,y-1]*G_neg

            elif not y == 0:
                if x == set['Nx']:
                    cijx_p, cijx_n, cijy_p, cijy_n = max_min_c(set,sol,x,y)
                    bijx_p, bijx_n, bijy_p, bijy_n = max_min_b(set,sol,x,y)
                    G_plus = chemo_coef*cijy_p-coef['Si']*bijy_p
                                
                    cijx_p, cijx_n, cijy_p, cijy_n = max_min_c(set,sol,x,y+2)
                    bijx_p, bijx_n, bijy_p, bijy_n = max_min_b(set,sol,x,y+2)
                    G_neg = chemo_coef*cijy_p-coef['Si']*bijy_p
                    
                    chemo_coef = coef['Ki_n']/(1+coef['Al_n']*1/4*(sol['c'][xb-2,yb-2]+sol['c'][xb-2,yb]+sol['c'][xb,yb-2]+sol['c'][xb,yb]))
                    
                    F_sol_2[x,y] = -coef['D_n']/(set['h'])*(sol['n'][x-1,y+1]-sol['n'][x-1,y-1])+sol['n'][x-1,y-1]*G_plus-sol['n'][x-1,y+1]*G_neg
                if not x == 0:
                    cijx_p, cijx_n, cijy_p, cijy_n = max_min_c(set,sol,x,y)
                    bijx_p, bijx_n, bijy_p, bijy_n = max_min_b(set,sol,x,y)
                    ...
                    cijx_p, cijx_n, cijy_p, cijy_n = max_min_c(set,sol,x+2,y)
                    bijx_p, bijx_n, bijy_p, bijy_n = max_min_b(set,sol,x+2,y)
                    G_neg_1 = chemo_coef*cijx_p-coef['Si']*bijx_p
                    
                    cijx_p, cijx_n, cijy_p, cijy_n = max_min_c(set,sol,x,y+2)
                    bijx_p, bijx_n, bijy_p, bijy_n = max_min_b(set,sol,x,y+2)
                    G_neg_2 = chemo_coef*cijy_p-coef['Si']*bijy_p
