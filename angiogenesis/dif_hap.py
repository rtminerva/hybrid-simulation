    ...
    for iy in range(Ny+1):
        ...
        if (iy==0):#boundary bawah
            for ix in range(Nx+1):
                ...
                P_3 = dt*D/dx**2 - dt*ro/(4*dx**2)*((f[ix,iy+1,it]-f[ix,iy,it]))
                P_4 = dt*D/dx**2 + dt*ro/(4*dx**2)*((f[ix,iy+1,it]-f[ix,iy,it]))
                if (ix==0):#pojok kiri bawah
                    P_1 = dt*D/dx**2 - dt/(4*dx**2)*(ro*(f[ix+1,iy,it]-f[ix,iy,it]))
                    P_2 = dt*D/dx**2 + dt/(4*dx**2)*(ro*(f[ix+1,iy,it]-f[ix,iy,it]))
                    P_0 = 1-4*dt*D/dx**2- dt*ro/dx**2*(f[ix+1,iy,it]+f[ix,iy,it]-4*f[ix,iy,it]+f[ix,iy+1,it]+f[ix,iy,it])
                    n[ix,iy,it+1] = P_0*n[ix,iy,it] + P_1*n[ix+1,iy,it] + P_2*n[ix,iy,it] + P_3*n[ix,iy+1,it] + P_4*n[ix,iy,it]
                elif (ix==Nx):#pojok kanan bawah
                    P_1 = dt*D/dx**2 - dt/(4*dx**2)*(ro*(f[ix,iy,it]-f[ix-1,iy,it]))
                    P_2 = dt*D/dx**2 + dt/(4*dx**2)*(ro*(f[ix,iy,it]-f[ix-1,iy,it]))
                    P_0 = 1-4*dt*D/dx**2 - dt*ro/dx**2*(f[ix,iy,it]+f[ix-1,iy,it]-4*f[ix,iy,it]+f[ix,iy+1,it]+f[ix,iy,it])
                    ...
                    P_1 = dt*D/dx**2 - dt/(4*dx**2)*(ro*(f[ix+1,iy,it]-f[ix-1,iy,it]))
                    P_2 = dt*D/dx**2 + dt/(4*dx**2)*(ro*(f[ix+1,iy,it]-f[ix-1,iy,it]))
                    P_0 = 1-4*dt*D/dx**2 - dt*ro/dx**2*(f[ix+1,iy,it]+f[ix-1,iy,it]-4*f[ix,iy,it]+f[ix,iy+1,it]+f[ix,iy,it])
                    n[ix,iy,it+1] = P_0*n[ix,iy,it] + P_1*n[ix+1,iy,it] + P_2*n[ix-1,iy,it] + P_3*n[ix,iy+1,it] + P_4*n[ix,iy-1,it]
        elif (iy==Ny):#boundary atas
            for ix in range(Nx+1):
                f[ix,iy,it+1] = f[ix,iy,it]*(1-dt*ga*n[ix,iy,it])+dt*be*n[ix,iy,it]
                P_3 = dt*D/dx**2 - dt/(4*dx**2)*(ro*(f[ix,iy,it]-f[ix,iy-1,it]))
                P_4 = dt*D/dx**2 + dt/(4*dx**2)*(ro*(f[ix,iy,it]-f[ix,iy-1,it]))
                if (ix==0):#pojok kiri atas
                    P_1 = dt*D/dx**2 - dt/(4*dx**2)*(ro*(f[ix+1,iy,it]-f[ix,iy,it]))
                    P_2 = dt*D/dx**2 + dt/(4*dx**2)*(ro*(f[ix+1,iy,it]-f[ix,iy,it]))
                    P_0 = 1-4*dt*D/dx**2 - dt*ro/dx**2*(f[ix+1,iy,it]+f[ix,iy,it]-4*f[ix,iy,it]+f[ix,iy,it]+f[ix,iy-1,it])
                    n[ix,iy,it+1] = P_0*n[ix,iy,it] + P_1*n[ix+1,iy,it] + P_2*n[ix,iy,it] + P_3*n[ix,iy,it] + P_4*n[ix,iy-1,it]
                elif (ix==Nx):#pojok kanan atas
                    P_1 = dt*D/dx**2 - dt/(4*dx**2)*(ro*(f[ix,iy,it]-f[ix-1,iy,it]))
                    P_2 = dt*D/dx**2 + dt/(4*dx**2)*(ro*(f[ix,iy,it]-f[ix-1,iy,it]))
                    P_0 = 1-4*dt*D/dx**2  - dt*ro/dx**2*(f[ix,iy,it]+f[ix-1,iy,it]-4*f[ix,iy,it]+f[ix,iy,it]+f[ix,iy-1,it])
                    ...
                    P_1 = dt*D/dx**2 - dt/(4*dx**2)*(ro*(f[ix+1,iy,it]-f[ix-1,iy,it]))
                    P_2 = dt*D/dx**2 + dt/(4*dx**2)*(ro*(f[ix+1,iy,it]-f[ix-1,iy,it]))
                    P_0 = 1-4*dt*D/dx**2 - dt*ro/dx**2*(f[ix+1,iy,it]+f[ix-1,iy,it]-4*f[ix,iy,it]+f[ix,iy,it]+f[ix,iy-1,it])
            ...
            for ix in range(Nx+1):
                f[ix,iy,it+1] = f[ix,iy,it]*(1-dt*ga*n[ix,iy,it])+dt*be*n[ix,iy,it]
                P_3 = dt*D/dx**2 - dt/(4*dx**2)*(ro*(f[ix,iy+1,it]-f[ix,iy-1,it]))
                P_4 = dt*D/dx**2 + dt/(4*dx**2)*(ro*(f[ix,iy+1,it]-f[ix,iy-1,it]))
                if (ix==0):#selain boundary atas bawah namun dipinggiran kiri
                    P_1 = dt*D/dx**2 - dt/(4*dx**2)*(ro*(f[ix+1,iy,it]-f[ix,iy,it]))
                    P_2 = dt*D/dx**2 + dt/(4*dx**2)*(ro*(f[ix+1,iy,it]-f[ix,iy,it]))
                    P_0 = 1-4*dt*D/dx**2 - dt*ro/dx**2*(f[ix+1,iy,it]+f[ix,iy,it]-4*f[ix,iy,it]+f[ix,iy+1,it]+f[ix,iy-1,it])
                    ...
                    P_1 = dt*D/dx**2 - dt/(4*dx**2)*(ro*(f[ix,iy,it]-f[ix-1,iy,it]))
                    P_2 = dt*D/dx**2 + dt/(4*dx**2)*(ro*(f[ix,iy,it]-f[ix-1,iy,it]))
                    P_0 = 1-4*dt*D/dx**2 - dt*ro/dx**2*(f[ix,iy,it]+f[ix-1,iy,it]-4*f[ix,iy,it]+f[ix,iy+1,it]+f[ix,iy-1,it])
                    ...
                    P_1 = dt*D/dx**2 - dt/(4*dx**2)*(ro*(f[ix+1,iy,it]-f[ix-1,iy,it]))
                    P_2 = dt*D/dx**2 + dt/(4*dx**2)*(ro*(f[ix+1,iy,it]-f[ix-1,iy,it]))
                    P_0 = 1-4*dt*D/dx**2 - dt*ro/dx**2*(f[ix+1,iy,it]+f[ix-1,iy,it]-4*f[ix,iy,it]+f[ix,iy+1,it]+f[ix,iy-1,it])
                    n[ix,iy,it+1] = P_0*n[ix,iy,it] + P_1*n[ix+1,iy,it] + P_2*n[ix-1,iy,it] + P_3*n[ix,iy+1,it] + P_4*n[ix,iy-1,it]
