...
def vector_A(theta = 0.5, time_step = 0.001, h = 0.2, X = 1, Y = 1, ccc = 0, fff = 0):
    lam = time_step/h**2
    hh = h/2
    Nx = int(X/hh)
    Ny = int(Y/hh)
...
def vector_B(theta = 0.5, time_step = 0.001, h = 0.2, X = 1, Y = 1, ccc = 0, fff = 0):
    lam = time_step/h**2
    hh = h/2
    Nx = int(X/hh)
    Ny = int(Y/hh)
    ...
    for i, x1 in enumerate(range(1,Nx,2)):
        B[i] = (P1_code(x = x1, y = j, c1 = ccc, f1 = fff)+P2_code(x = x1, y = j, c1 = ccc, f1 = fff)+P3_code(x = x1, y = j, c1 = ccc, f1 = fff))*theta*lam
    '''Inside'''
    for j, y1 in enumerate(range(3,Ny-1,2)):
        for i, x1 in enumerate(range(1,Nx,2)):
            B[(i+Nx/2)+j*Nx/2] = (P1_code(x = x1, y = y1, c1 = ccc, f1 = fff) + P2_code(x = x1, y = y1, c1 = ccc, f1 = fff) + P3_code(x = x1, y = y1, c1 = ccc, f1 = fff) + P4_code(x = x1, y = y1, c1 = ccc, f1 = fff))*theta*lam
    '''Up side'''
    j = Ny-1
    for i, x1 in enumerate(range(1,Nx,2)):
        B[i+(Nx/2)**2-Nx/2] = (P1_code(x = x1, y = j, c1 = ccc, f1 = fff)+P2_code(x = x1, y = j, c1 = ccc, f1 = fff)+P4_code(x = x1, y = j, c1 = ccc, f1 = fff))*theta*lam
    return B

    
def vector_C(theta = 0.5, time_step = 0.001, h = 0.2, X = 1, Y = 1, ccc = 0, fff = 0):
    lam = time_step/h**2
    hh = h/2
    Nx = int(X/hh)
    Ny = int(Y/hh)
...
def vector_D(theta = 0.5, time_step = 0.001, h = 0.2, X = 1, Y = 1, ccc = 0, fff = 0):
    lam = time_step/h**2
    hh = h/2
    Nx = int(X/hh)
    Ny = int(Y/hh)
...
def vector_E(theta = 0.5, time_step = 0.001, h = 0.2, X = 1, Y = 1, ccc = 0, fff = 0):
    lam = time_step/h**2
    hh = h/2
    Nx = int(X/hh)
    Ny = int(Y/hh)
...
#                 n[x,y] = m.exp(-(x*hh)**2/0.001)*(m.sin(number_of_tip*m.pi*y*hh))**2
    ...
    '''Build SPL Matrix'''
    teta = 0.75
    A_left = vector_A(ccc = c, fff = f, theta = teta)
    B_left = vector_B(ccc = c, fff = f, theta = teta)
    C_left = vector_C(ccc = c, fff = f, theta = teta)
    D_left = vector_D(ccc = c, fff = f, theta = teta)
    E_left = vector_E(ccc = c, fff = f, theta = teta)
    
    A_right = vector_A(ccc = c, fff = f, theta = 1-teta)
    B_right = vector_B(ccc = c, fff = f, theta = 1-teta)
    C_right = vector_C(ccc = c, fff = f, theta = 1-teta)
    D_right = vector_D(ccc = c, fff = f, theta = 1-teta)
    E_right = vector_E(ccc = c, fff = f, theta = 1-teta)
