...
def vector_A(theta = 0.5, time_step = 0.1, h = 0.005, X = 1, Y = 1, ccc = 0, fff = 0, left = False):
    lam = time_step/h**2
    hh = h/2
    Nx = int(X/hh)
    Ny = int(Y/hh)
...
def vector_B(theta = 0.5, time_step = 0.1, h = 0.005, X = 1, Y = 1, ccc = 0, fff = 0, left = False):
    lam = time_step/h**2
    hh = h/2
    Nx = int(X/hh)
    Ny = int(Y/hh)
...
def vector_C(theta = 0.5, time_step = 0.1, h = 0.005, X = 1, Y = 1, ccc = 0, fff = 0, left = False):
    lam = time_step/h**2
    hh = h/2
    Nx = int(X/hh)
    Ny = int(Y/hh)
...
def vector_D(theta = 0.5, time_step = 0.1, h = 0.005, X = 1, Y = 1, ccc = 0, fff = 0, left = False):
    lam = time_step/h**2
    hh = h/2
    Nx = int(X/hh)
    Ny = int(Y/hh)
...
def vector_E(theta = 0.5, time_step = 0.1, h = 0.005, X = 1, Y = 1, ccc = 0, fff = 0, left = False):
    lam = time_step/h**2
    hh = h/2
    Nx = int(X/hh)
    Ny = int(Y/hh)
...
#                 n[x,y] = m.exp(-(x*hh)**2/0.001)*(m.sin(number_of_tip*m.pi*y*hh))**2
    ...
    for y in range(2,Ny+1,2):
        for x in range(0,Nx,2):
            ...
            v2.append(max(1/(2*h)*(c_o[x+2,y]-c_o[x,y]+c_o[x+2,y-2]-c_o[x,y-2]),1/(2*h)*(c_o[x+2,y]-c_o[x+2,y-2]+c_o[x,y]-c_o[x,y-2])))
            w2.append(max(1/(2*h)*(f_o[x+2,y]-f_o[x,y]+f_o[x+2,y-2]-f_o[x,y-2]),1/(2*h)*(f_o[x+2,y]-f_o[x+2,y-2]+f_o[x,y]-f_o[x,y-2])))
            w1.append(max(1/(2*h)*(f[x+2,y]-f[x,y]+f[x+2,y-2]-f[x,y-2]),1/(2*h)*(f[x+2,y]-f[x+2,y-2]+f[x,y]-f[x,y-2])))
    tau1 = min(h**2/(4*teta*(d+h*ki*max(v1)/(1+al*c.max())+h*ro*max(w1))),h**2/(4*(1-teta)*(d+h*ki*max(v2)/(1+al*c_o.max())+h*ro*max(w2))),1/(nu*n.max()))
    if tau1 < tp:
        tp = tau1
    del v1
    del v2
    ...
    data = numpy.array([vector_D(ccc = c_o, fff = f_o, theta = 1-teta, time_step = tp), vector_A(ccc = c_o, fff = f_o, theta = 1-teta, time_step = tp), vector_B(ccc = c_o, fff = f_o, theta = 1-teta, time_step = tp), vector_C(ccc = c_o, fff = f_o, theta = 1-teta, time_step = tp), vector_E(ccc = c_o, fff = f_o, theta = 1-teta, time_step = tp)])
...
#     print len(n_v)
    ...
    data = numpy.array([vector_D(ccc = c_o, fff = f_o, theta = teta, left = True, time_step = tp), vector_A(ccc = c_o, fff = f_o, theta = teta, left = True, time_step = tp), vector_B(ccc = c_o, fff = f_o, theta = teta, left = True, time_step = tp), vector_C(ccc = c_o, fff = f_o, theta = teta, left = True, time_step = tp), vector_E(ccc = c_o, fff = f_o, theta = teta, left = True, time_step = tp)])
