...
#             sol['n'][x] = 0.25*m.exp(-(x*set['Hh']-set['rad']+0.03)**2/0.007) #0.25      
        sol['n'][x] = 0.5*m.exp(-(x*set['Hh']-set['rad'])**2/0.002) #0.002 #0.35 #x*set['Hh']-set['rad']
#         sol['n'][x] = 0.2 + 0.2*m.tanh(((set['rad'])-x*set['Hh'])/0.01)
    return sol

def b_prof(coef,set,sol):
    for x in range(1,set['Nx'],2):
        sol['b'][x] = 0.5 + 0.5*m.tanh(((set['rad']-0.05)-x*set['Hh'])/0.01) #0.01
