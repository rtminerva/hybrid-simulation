...
def sol_A_I_Ki(coef,set,sol,A_o,I_o,Ki_o, c_o):
    for x in range(1,set['Nx'],2):
        ...
        #basic
#         sol['Ki_f'][x] = (coef['k_A']*sol['A'][x]*(coef['Ki_tot']-Ki_o[x])-coef['k_I']*Ki_o[x]*sol['I'][x])
        sol['Ki'][x] = Ki_o[x] + set['dt']*(coef['k_A']*sol['A'][x]*(coef['Ki_tot']-Ki_o[x])-coef['k_I']*Ki_o[x]*sol['I'][x])
        #ultrasensitif
#         sol['Ki_f'][x] = (coef['k_A']*sol['A'][x]*(coef['Ki_tot']-Ki_o[x])/((coef['Ki_tot']-Ki_o[x])+coef['K_A'])-coef['k_I']*Ki_o[x]*sol['I'][x]/(Ki_o[x]+coef['K_I']))
...
#     if set['t']>0.12 and set['k']% 20:
#         print '****', set['t']
#         for x in range(1,set['Nx'],2):
#             print 'Ki response', sol['Ki'][x]
    return sol  
    ...
    for x in range(0,set['Nx']+1,2):
        if not x == 0:
            if not x == set['Nx']:
                ...
                G_plus_1 = (sol['Ki_f'][x-1])*cijx_p
                
#                 if set['k'] > 50:#sol['Ki'][x-1] < 0.33:
#                     print 'Ki response', sol['Ki'][x-1]
                
                cijx_p, cijx_n = max_min_c(set,sol,x+2,c_o) #2.3.(1).(1)
                G_neg_1 = (sol['Ki_f'][x+1])*cijx_n
