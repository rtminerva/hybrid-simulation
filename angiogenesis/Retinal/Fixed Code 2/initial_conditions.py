...
def initial_prof(coef, set, sol):  
    ...
    if not coef['Mic'] == 0 or not coef['Kappa'] == 0:
        ...
        for tt in range(0,250):
            idx_m_1 = random.sample(range(1,440),100)
            idx_m_2 = random.sample(range(1,440),100)
            for id in range(0,len(idx_m_1)):
                r_f = numpy.sqrt((idx_m_1[id]*set['h']-set['O_x'])**2 + (idx_m_2[id]*set['h']-set['O_y'])**2)
