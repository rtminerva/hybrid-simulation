...
def P1_code(h1 = 0,d = 0.00035, ki = 0.38, al = 0.6, ro2 = 0, c1 = 0, x = 0, y = 0, f1 = 0):
    r = d + h1*ki/(1+al*c1[x-1,y+1])*max(-vx_code(x_p = x, y_p = y, cc = c1), 0) + h1*ro2*max(-wx_code(x_p = x, y_p = y, ff = f1), 0)
    return r

def P2_code(h1 = 0,d = 0.00035, ki = 0.38, al = 0.6, ro2 = 0, c1 = 0, x = 0, y = 0, f1 = 0):
    r = d + h1*ki/(1+al*c1[x+1,y+1])*max(vx_code(x_p = x, y_p = y, cc = c1), 0) + h1*ro2*max(wx_code(x_p = x, y_p = y, ff = f1), 0)
    return r

def P3_code(h1 = 0,d = 0.00035, ki = 0.38, al = 0.6, ro2 = 0, c1 = 0, x = 0, y = 0, f1 = 0):
    r = d + h1*ki/(1+al*c1[x+1,y-1])*max(-vy_code(x_p = x, y_p = y, cc = c1), 0) + h1*ro2*max(-wy_code(x_p = x, y_p = y, ff = f1), 0)
    return r

def P4_code(h1 = 0,d = 0.00035, ki = 0.38, al = 0.6, ro2 = 0, c1 = 0, x = 0, y = 0, f1 = 0): #at main lattice
    r = d + h1*ki/(1+al*c1[x+1,y+1])*max(vy_code(x_p = x, y_p = y, cc = c1), 0) + h1*ro2*max(wy_code(x_p = x, y_p = y, ff = f1), 0)
    return r
...
def vector_B(theta = 0, time_step = 0, h2 = 0, ro1 = 0, Nx1 = 1, Ny1 = 1, ccc = 0, fff = 0, left = False, d1 =0, ki1=1, al1=0):
    ...
    if left == True: # I + matrix
        ...
        for j, y1 in enumerate(range(3,Ny1-1,2)):
            x1 = 1
            i = 0
            B[(i+Nx1/2)+j*Nx1/2] = 1 + (P2_code(x = x1, y = y1, c1 = ccc, f1 = fff, h1 = h2, ro2 = ro1) + P3_code(x = x1, y = y1, c1 = ccc, f1 = fff, h1 = h2, ro2 = ro1) + P4_code(x = x1, y = y1, c1 = ccc, f1 = fff, h1 = h2, ro2 = ro1))*theta*lam
            for x1 in range(3,Nx1-1,2):
                i += 1
                B[(i+Nx1/2)+j*Nx1/2] = 1 + (P1_code(x = x1, y = y1, c1 = ccc, f1 = fff, h1 = h2, ro2 = ro1) + P2_code(x = x1, y = y1, c1 = ccc, f1 = fff, h1 = h2, ro2 = ro1) + P3_code(x = x1, y = y1, c1 = ccc, f1 = fff, h1 = h2, ro2 = ro1) + P4_code(x = x1, y = y1, c1 = ccc, f1 = fff, h1 = h2, ro2 = ro1))*theta*lam
            x1 = Nx1-1
            B[(i+1+Nx1/2)+j*Nx1/2] = 1 + (P1_code(x = x1, y = y1, c1 = ccc, f1 = fff, h1 = h2, ro2 = ro1) + P3_code(x = x1, y = y1, c1 = ccc, f1 = fff, h1 = h2, ro2 = ro1) + P4_code(x = x1, y = y1, c1 = ccc, f1 = fff, h1 = h2, ro2 = ro1))*theta*lam
        '''Up side'''
        y1 = Ny1-1
        x1 = 1
        i = 0
        B[i+(Nx1/2)**2-Nx1/2] = 1 + (P2_code(x = x1, y = y1, c1 = ccc, f1 = fff, h1 = h2, ro2 = ro1)+P3_code(x = x1, y = y1, c1 = ccc, f1 = fff, h1 = h2, ro2 = ro1))*theta*lam
        for x1 in range(3,Nx1-1,2):
            i += 1
            B[i+(Nx1/2)**2-Nx1/2] = 1 + (P1_code(x = x1, y = y1, c1 = ccc, f1 = fff, h1 = h2, ro2 = ro1)+P2_code(x = x1, y = y1, c1 = ccc, f1 = fff, h1 = h2, ro2 = ro1)+P3_code(x = x1, y = y1, c1 = ccc, f1 = fff, h1 = h2, ro2 = ro1))*theta*lam
        x1 = Nx1-1
        B[i+1+(Nx1/2)**2-Nx1/2] = 1 + (P1_code(x = x1, y = y1, c1 = ccc, f1 = fff, h1 = h2, ro2 = ro1)+P3_code(x = x1, y = y1, c1 = ccc, f1 = fff, h1 = h2, ro2 = ro1))*theta*lam
        ...
        for j, y1 in enumerate(range(3,Ny1-1,2)):
            x1 = 1
            i = 0
            B[(i+Nx1/2)+j*Nx1/2] = 1 - (P2_code(x = x1, y = y1, c1 = ccc, f1 = fff, h1 = h2, ro2 = ro1) + P3_code(x = x1, y = y1, c1 = ccc, f1 = fff, h1 = h2, ro2 = ro1) + P4_code(x = x1, y = y1, c1 = ccc, f1 = fff, h1 = h2, ro2 = ro1))*(1-theta)*lam
            for x1 in range(3,Nx1-1,2):
                i += 1
                B[(i+Nx1/2)+j*Nx1/2] = 1 - (P1_code(x = x1, y = y1, c1 = ccc, f1 = fff, h1 = h2, ro2 = ro1) + P2_code(x = x1, y = y1, c1 = ccc, f1 = fff, h1 = h2, ro2 = ro1) + P3_code(x = x1, y = y1, c1 = ccc, f1 = fff, h1 = h2, ro2 = ro1) + P4_code(x = x1, y = y1, c1 = ccc, f1 = fff, h1 = h2, ro2 = ro1))*(1-theta)*lam
            x1 = Nx1-1
            B[(i+1+Nx1/2)+j*Nx1/2] = 1 - (P1_code(x = x1, y = y1, c1 = ccc, f1 = fff, h1 = h2, ro2 = ro1) + P3_code(x = x1, y = y1, c1 = ccc, f1 = fff, h1 = h2, ro2 = ro1) + P4_code(x = x1, y = y1, c1 = ccc, f1 = fff, h1 = h2, ro2 = ro1))*(1-theta)*lam
        '''Up side'''
        y1 = Ny1-1
        x1 = 1
        i = 0
        B[i+(Nx1/2)**2-Nx1/2] = 1 - (P2_code(x = x1, y = y1, c1 = ccc, f1 = fff, h1 = h2, ro2 = ro1)+P3_code(x = x1, y = y1, c1 = ccc, f1 = fff, h1 = h2, ro2 = ro1))*(1-theta)*lam
        for x1 in range(3,Nx1-1,2):
            i += 1
            B[i+(Nx1/2)**2-Nx1/2] = 1 - (P1_code(x = x1, y = y1, c1 = ccc, f1 = fff, h1 = h2, ro2 = ro1)+P2_code(x = x1, y = y1, c1 = ccc, f1 = fff, h1 = h2, ro2 = ro1)+P3_code(x = x1, y = y1, c1 = ccc, f1 = fff, h1 = h2, ro2 = ro1))*(1-theta)*lam
        x1 = Nx1-1
        B[i+1+(Nx1/2)**2-Nx1/2] = 1 - (P1_code(x = x1, y = y1, c1 = ccc, f1 = fff, h1 = h2, ro2 = ro1)+P3_code(x = x1, y = y1, c1 = ccc, f1 = fff, h1 = h2, ro2 = ro1))*(1-theta)*lam
...
def continuous_sparse_matrix_1_iter(teta = 0,
    ...
    A_right = vector_A(ccc = p_o, fff = fake, theta = teta, time_step = tp, h2 = h3, ro1 = ro, d1 = d_m, ki1 = ki_m, al1 = al_m)
    B_right = vector_B(ccc = p_o, fff = fake, theta = teta, time_step = tp, h2 = h3, ro1 = ro, d1 = d_m, ki1 = ki_m, al1 = al_m)
    C_right = vector_C(ccc = p_o, fff = fake, theta = teta, time_step = tp, h2 = h3, ro1 = ro, d1 = d_m, ki1 = ki_m, al1 = al_m)
    D_right = vector_D(ccc = p_o, fff = fake, theta = teta, time_step = tp, h2 = h3, ro1 = ro, d1 = d_m, ki1 = ki_m, al1 = al_m)
    E_right = vector_E(ccc = p_o, fff = fake, theta = teta, time_step = tp, h2 = h3, ro1 = ro, d1 = d_m, ki1 = ki_m, al1 = al_m)
    ...
    data = numpy.array([vector_D(ccc = p_o, fff = fake, theta = teta, left = True, time_step = tp, h2 = h3, ro1 = ro, d1 = d_m, ki1 = ki_m, al1 = al_m), vector_A(ccc = p_o, fff = fake, theta = teta, left = True, time_step = tp, h2 = h3, ro1 = ro, d1 = d_m, ki1 = ki_m, al1 = al_m), vector_B(ccc = p_o, fff = fake, theta = teta, left = True, time_step = tp, h2 = h3, ro1 = ro, d1 = d_m, ki1 = ki_m, al1 = al_m), vector_C(ccc = p_o, fff = fake, theta = teta, left = True, time_step = tp, h2 = h3, ro1 = ro, d1 = d_m, ki1 = ki_m, al1 = al_m), vector_E(ccc = p_o, fff = fake, theta = teta, left = True, time_step = tp, h2 = h3, ro1 = ro, d1 = d_m, ki1 = ki_m, al1 = al_m)])
        ...
        for x in range(0,Nx+1,2):
            ...
            if r_f <= (r_min**2 + Error + hh):
                if x >= matrix_tip[2][0][0] and y >= matrix_tip[0][0][1]: #area 1
                    ...
                    p[x,y] = a_p*1/3*(m[x-1,y+1]+m[x+1,y+1]+m[x+1,y-1])+b_p #Ang1
                    #p[x,y] = 1/(a_p*1/3*(m[x-1,y+1]+m[x+1,y+1]+m[x+1,y-1])+b_p) #Ang2
