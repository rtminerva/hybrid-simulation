    ...
    if iter == 1:     
        ...
        for y in range(0,Ny+1,2):
            for x in range(0,Nx+1,2):
                f[x,y] = 1-0.45*m.exp(-(0.5)**2/0.45)
                #f[x,y] = 0.75*m.exp(-(x*hh)**2/e)    
        sp_stop = []
        n = numpy.zeros((Nx+1,Ny+1))
                
        '''Initial Tips'''
        matrix_tip =[[(Nx/2-1,Ny/2-1)], [(Nx/2-1,Ny/2+1)], [(Nx/2+1,Ny/2-1)], [(Nx/2+1,Ny/2+1)]] #real time position
        n[Nx/2-1,Ny/2-1] = 1
        n[Nx/2-1,Ny/2+1] = 1
        n[Nx/2+1,Ny/2-1] = 1
        n[Nx/2+1,Ny/2+1] = 1
        list_last_movement =['start', 'start', 'start','start'] #last tip movement
        list_tip_movement =['start', 'start', 'start','start']  #movement tip
        life_time_tip=[0,0,0,0] #lifetime
        '''Initial Tips'''
