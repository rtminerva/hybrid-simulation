...
def continuous_sparse_matrix_1_iter(teta = 0.75,d = 0.00035, dc = 0.00035, ki = 0.38,al = 0.6,ro = 0,
...
#    from numpy.linalg import solve
    ...
    for y in range(0,Ny+1,2):
        for x in range(0,Nx+1,2):
            if y == 0:
                if x == 0:
                    c[x,y] = c_o[x,y]*(1 - tp*nu*n[1,1]) + dc/h3 * (vx_code(x_p = x+1, y_p = y+1, cc = c_o)+vy_code(x_p = x+1, y_p = y+1, cc = c_o))
                    f[x,y] = f_o[x,y]+ tp*(be-ga*f_o[x,y])*n[1,1]
                elif x == Nx:
                    c[x,y] = c_o[x,y]*(1 - tp*nu*n[Nx-1,1]) + dc/h3 * (-vx_code(x_p = x-1, y_p = y+1, cc = c_o))
                    f[x,y] = f_o[x,y]+ tp*(be-ga*f_o[x,y])*n[Nx-1,1]
                else:
                    c[x,y] = c_o[x,y]*(1 - tp*nu*0.5*(n[x+1,1]+n[x-1,1])) + dc/h3 * (vx_code(x_p = x+1, y_p = y+1, cc = c_o)-vx_code(x_p = x-1, y_p = y+1, cc = c_o)+vy_code(x_p = x+1, y_p = y+1, cc = c_o))
                    f[x,y] = f_o[x,y]+ tp*(be-ga*f_o[x,y])*0.5*(n[x+1,1]+n[x-1,1])
            elif y == Ny:
                if x == 0:
                    c[x,y] = c_o[x,y]*(1 - tp*nu*n[1,Ny-1]) + dc/h3 * (-vy_code(x_p = x+1, y_p = y-1, cc = c_o))
                    f[x,y] = f_o[x,y]+ tp*(be-ga*f_o[x,y])*n[1,Ny-1]
                elif x == Nx:
                    c[x,y] = c_o[x,y]*(1 - tp*nu*n[Nx-1,Ny-1]) + dc/h3 * (0)
                    f[x,y] = f_o[x,y]+ tp*(be-ga*f_o[x,y])*n[Nx-1,Ny-1]
                else:
                    c[x,y] = c_o[x,y]*(1 - tp*nu*0.5*(n[x+1,Ny-1]+n[x-1,Ny-1])) + dc/h3 * (-vy_code(x_p = x+1, y_p = y-1, cc = c_o))
                    f[x,y] = f_o[x,y]+ tp*(be-ga*f_o[x,y])*0.5*(n[x+1,Ny-1]+n[x-1,Ny-1])
            else:
                if x == 0:
                    c[x,y] = c_o[x,y]*(1 - tp*nu*0.5*(n[x+1,y+1]+n[x+1,y-1])) + dc/h3 * (vx_code(x_p = x+1, y_p = y+1, cc = c_o)+vy_code(x_p = x+1, y_p = y+1, cc = c_o)-vy_code(x_p = x+1, y_p = y-1, cc = c_o)) 
                    f[x,y] = f_o[x,y]+ tp*(be-ga*f_o[x,y])*0.5*(n[x+1,y+1]+n[x+1,y-1])
                elif x == Nx:
                    c[x,y] = c_o[x,y]*(1 - tp*nu*0.5*(n[x-1,y+1]+n[x-1,y-1])) + dc/h3 * (-vx_code(x_p = x-1, y_p = y+1, cc = c_o))
                    f[x,y] = f_o[x,y]+ tp*(be-ga*f_o[x,y])*0.5*(n[x-1,y+1]+n[x-1,y-1])
                else:
                    c[x,y] = c_o[x,y]*(1 - tp*nu*0.25*(n[x+1,y+1]+n[x-1,y+1]+n[x+1,y-1]+n[x-1,y-1])) + dc/h3 * (vx_code(x_p = x+1, y_p = y+1, cc = c_o)-vx_code(x_p = x-1, y_p = y+1, cc = c_o)+vy_code(x_p = x+1, y_p = y+1, cc = c_o)-vy_code(x_p = x+1, y_p = y-1, cc = c_o))
